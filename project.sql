...
-- Database Creation 
-- *********************************************
create database last_mile_delivery_analysis;
use last_mile_delivery_analysis;
...
UPDATE deliveries SET actual_delivery_date = NULL
...
select service_type from orders group by service_type;

-- 5.	How many drivers are currently active?

select count(driver_name) from drivers where is_active='Yes';

-- 6.	What are the different vehicle types?

select vehicle_type from vehicles group by vehicle_type;

-- 7.	What is the total order value?

select customer_id,total_value from orders;
...
-- 8. Which vehicles have the highest average delivery distance?
select vehicle_id,round(avg(distance_km),2) as avg_distance from deliveries
group by vehicle_id
order by avg_distance desc;

-- 9. Which drivers have an average rating above 4?
select driver_id, driver_name, rating from drivers
where rating > 4
order by rating desc;

-- 10. Which vehicle types have the highest average payload capacity?
